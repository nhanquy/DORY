...
//
Layering::Layering() :
		feasible(true),  nTasks(0), timeHorizon(0), max_resource(0)
{
}
...
QNum Layering::get_peak(int start, int end, numVec res_row, QNum& sum)
{
	if (end >= res_row.size())
	{
		DEBUG_LOG << "res_row out of rank: ending at "<<end<<" instead of "<<res_row.size();
		feasible = false;
		return -1;
	}
		...
		QNum res_ub, QInt releaseDate, QInt dueDate)
{
	...
	for (QInt dur=dur_max; dur>=dur_min; --dur)
	{
		for(QInt start=releaseDate; start<=dueDate-dur; ++start)
		{
			...
			if ((peak<minmaxPeak)||(peak == minmaxPeak && sum<minmaxSum))
			{
				minmaxPeak = peak;
				minmaxSum = sum;
...
void Layering::allocate_resource(int start,int end,QNum res,numVec& res_row)
{
	int size = res_row.size();
	for(int i=0; i<size; ++i)
		if(i>=start && i<=end) res_row[i]=res;
		else res_row[i]=0;
}
...
void Layering::solve(Input params,std::vector<int> tasks_queue)
{
	...
	if (feasible)
	{
		DEBUG_LOG << "\nLayering done! Displaying result: ";
		disp_matrix(resource_allocation, "Resource allocation");
	}
	else
	{
		DEBUG_LOG << "\nUnable to find solution! Displaying temporal result: ";
		disp_matrix(resource_allocation, "Resource allocation");
	}
}
