...
bool TimeRef::isValideTimeStr(const timestr& to_valide) const {
	std::stringstream ss;
	const char delim = ':';
	char c1, c2;
	int hr, min, sec;
	...
	if ((ss >> hr >> c1 >> min >> c2 >> sec) && (c1 == delim) && (c1 == c2)
		...
		LOG(ERROR) <<to_valide<< ": Time String invalid";
		return false;
	}
}

bool TimeRef::isInHorizon(const timestr& to_valide) const {
	double t1 = difference(start_horizon, to_valide);
	double t2 = difference(to_valide, end_horizon);
	double t3 = hours_end();
	return (t1+t2<=t3+0.0001);
}
...
bool TimeRef::timestr_parser(const timestr& tstring, int& hh, int& mm, int& ss) const {
